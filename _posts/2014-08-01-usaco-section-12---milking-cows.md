...
int main()
{
    ...
    int n, i, j, st, eat, noeat, ed;
    scanf("%d", &n);
    for (i = 0; i < n; i++)
        scanf("%d%d", &cow[i].st, &cow[i].ed);
    sort(cow, cow + n);
    st = cow[0].st, ed = cow[0].ed, eat = ed - st, noeat = 0;
    for (i = 1; i < n; i++)
    {
        if (cow[i].st <= ed)
            ed = max(ed, cow[i].ed);
        ...
        {
            eat = max(eat, ed - st);
            noeat = max(noeat, cow[i].st - ed);
            st = cow[i].st, ed = cow[i].ed;
        }
    }
​    eat = max(eat, cow[i - 1].ed - st);
    printf("%d %d\n", eat, noeat);
    return 0;
}
```
...
int main()
{
    ...
    for (i = 0; i < n; i++)
        scanf("%d%d", &cow[i].st, &cow[i].ed);
    sort(cow, cow + n);
    st = cow[0].st, eat = cow[0].ed - cow[0].st, noeat = 0, ed = 0;
    vis[st] = 1;
    for (i = 0; i < n; i++)
    {
        if (!vis[cow[i].st])
