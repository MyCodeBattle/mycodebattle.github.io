...
int mp[50][50], num[50][50], nmat, nrem;
...
{
    //ROP;
    int row, col, nhole, i, j;
    while (~scanf("%d%d%d", &row, &col, &nhole))
    {
        MS(mp, 0); MS(num, 0);
        hun.init();
        int x, y;
        ...
        for (i = 1; i <= row; i++)
            for (j = 1; j <= col; j++)
            {
                ...
                for (int k = 0; k < 4; k++)
                {
                    ...
                    if (xx >= 1 && xx <= row && yy >= 1 && yy <= col && !mp[xx][yy])
                        hun.add_edge(num[i][j], num[xx][yy]);
                }
            }
        printf("%s\n", (hun.hungary() == cnt ? "YES" : "NO"));
    }
    return 0;
}
```