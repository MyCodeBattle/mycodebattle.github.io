...
class RockPaperScissorsMagicEasy {
    ...
    int count(vector<int> c, int score) {
        ...
        reverse(c.begin(), c.end());
        int sz = SZ(c);
        if (c[1] == 1) dp[1][2][1] = 1, dp[1][1][0] = 1, dp[1][0][0] = 1;
        else if (c[1] == 0) dp[1][1][1] = 1, dp[1][0][0] = 1, dp[1][2][0] = 1;
        else if (c[1] == 2) dp[1][0][1] = 1, dp[1][1][0] = 1, dp[1][2][0] = 1;
        for (int i = 2; i < sz; i++)
        {
            for (int j = 0; j < 3; j++)
            {
                for (int k = score; k >= 0; k--)
                {
                    if (j == 0)
                    {
                        ...
                        {
                            if (k == 0) break;
                            dp[i][j][k] = (dp[i-1][0][k-1] + dp[i-1][1][k-1]) % MOD + dp[i-1][2][k-1], dp[i][j][k] %= MOD;
                        }
                        else
                            for (int l = 0; l < 3; l++) dp[i][j][k] = (dp[i][j][k] + dp[i-1][l][k]) % MOD;
                    }
                    if (j == 1)
                    {
                        if (c[i] == 0)
                        {
                            if (k == 0) break;
                            for (int l = 0; l < 3; l++) (dp[i][j][k] = dp[i-1][l][k-1] + dp[i][j][k]) % MOD;
                        }
                        else
                            for (int l = 0; l < 3; l++) dp[i][j][k] = (dp[i-1][l][k] + dp[i][j][k]) % MOD;
                    }
                    if (j == 2)
                    {
                        if (c[i] == 1)
