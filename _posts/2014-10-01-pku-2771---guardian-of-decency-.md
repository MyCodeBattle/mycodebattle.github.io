...
{
    if (abs(peo[i].age - peo[j].age) > 40 || peo[i].sex == peo[j].sex
        || peo[i].hb == peo[j].hb || peo[i].mus != peo[j].mus) return false;
    return true;
}
...
int main()
{
    ...
    while (T--)
    {
        ...
        for (i = 1; i <= n; i++)
        {
            cin >> peo[i].age >> peo[i].sex >> peo[i].mus >> peo[i].hb;
            for (j = 1; j < i; j++)
                if (Check(i, j))
                {
                    hun.add_edge(j, i);
