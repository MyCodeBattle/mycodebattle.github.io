...
struct TRIE
{
    int ch[MAXN][3], v[MAXN], sz;
 
    int get_idx(char c) { return c-'a'; }
    void init()
    {
        sz = 1;
        MS(ch[0], 0);
    ...
    bool find(int node, int len, int flag)
    {
        if (str[len] == 0)
        {
            if (flag && v[node] == 1)
        ...
        if (flag)
        {
            if (ch[node][get_idx(str[len])] != 0 && find(ch[node][get_idx(str[len])], len+1, flag)) return true;
            return false;
        }
        else
        {
            if (ch[node][get_idx(str[len])] != 0)
                if (find(ch[node][get_idx(str[len])], len+1, flag))return true;
            for (int i = 0; i < 3; i++)
            {
                if (i == get_idx(str[len])) continue;
                if (ch[node][i] != 0)
                    if (find(ch[node][i], len+1, flag+1)) return true;
            }
            return false;
        }
    }
