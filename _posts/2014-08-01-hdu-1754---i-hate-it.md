...
void PushUp(int cur)
{
    segt[cur].vmax = max(segt[cur << 1].vmax, segt[cur << 1 | 1].vmax);
}
...
int Query(int cur, int l, int r)
{
    ...
    if (segt[cur].l == l && segt[cur].r == r)
        return segt[cur].vmax;
    if (r <= mid)
        ans = max(Query(cur << 1, l, r), ans);
    else if (l > mid)
        ans = max(ans, Query(cur << 1 | 1, l, r));
    else
    {
        x = Query(cur << 1, l, mid);
        y = Query(cur << 1 | 1, mid + 1, r);
    }
