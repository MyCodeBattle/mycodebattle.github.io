...
#  [UVa 10820 - Send a Table](/2014/07/UVa-10820/ "UVa 10820 - Send a Table")

By [MyCodeBattle](http://mycodebattle.gitcafe.io/about "MyCodeBattle")

Published Jul 28 2014 22:33
...
| 
    ...
    #include <bits/stdc++.h>using namespace std;const int MAXN = 50000 + 100;const int INF = 0x3f3f3f3f; int phi[MAXN], sum[MAXN]; void PHI_Table(){    for (int i = 2; i < MAXN; ++i)        if (!phi[i])            for (int j = i; j < MAXN; j += i)            {                if (!phi[j])                    phi[j] = j;                phi[j] = phi[j] / i * (i - 1);            }} int main(){    int n;    PHI_Table();    sum[0] = 1, phi[1] = 1;    for (int i = 1; i <= MAXN; i++)        sum[i] = sum[i - 1] + phi[i];    while (~scanf("%d", &n), n)        printf("%d\n", 2 * sum[n] - 3);    return 0;}  
